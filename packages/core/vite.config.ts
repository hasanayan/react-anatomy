...
// solver worker: `slot-annotations.tsx` imports it with `?worker&inline`, so the
// worker's code lands in `index.js` as a base64 blob and no separate worker file
// ships. That removes the `new URL(...)` worker file that made every consuming
// host add `optimizeDeps.exclude`. Types are emitted separately by tsc
