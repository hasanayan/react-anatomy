...
// A solver that fails the way a hard rule (§9/§10) does: the answer rejects.
function rejectingSolver(message: string): Solver {
  return {
    solve(): Promise<Placement | null> {
...
// Catches the render throw so the test can read the message the reader would.
...
> {
  ...
  override render(): ReactNode {
    return this.state.message === null ? (
      ...
      <div data-caught={this.state.message} />
    );
  }
}

describe("SlotAnnotations hard-rule errors", () => {
  it("rethrows a solve rejection from render so the story dies loudly", async () => {
    // React logs a caught render error; keep the test output clean.
    const errorLog = vi.spyOn(console, "error").mockImplementation(() => {});

    const { container } = await render(
      <Boundary>
        <SlotAnnotations solver={rejectingSolver("[anatomy] rule 9 violated")}>
          {card()}
        </SlotAnnotations>
      </Boundary>,
    );
    ...
    expect(caught?.getAttribute("data-caught")).toBe(
      "[anatomy] rule 9 violated",
    );
