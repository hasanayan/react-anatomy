...
import type { LabelSize, Rect } from "./place-labels";
import type { Placement, PlacedLabel } from "./zones";
...
function placementOf(regions: Region[]): Placement {
  const labels: PlacedLabel[] = regions.map((r, index) => ({
    index,
    zoneId: r.id,
    region: r,
    side: "left",
