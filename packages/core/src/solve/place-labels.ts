...
export function placeZones(
  ...
  const labels: PlacedLabelData[] = placed.map((item) => ({
    zoneId: item.zoneId,
    side: item.side,
    ...
    // Rounded here so the drawn leader is in §6's "round only at the end" form
    // and no draw site re-rounds.
  ...
  const frames: PlacedFrameData[] = zones.map((zone) => ({
    zoneId: zone.id,
